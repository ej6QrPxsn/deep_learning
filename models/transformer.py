...
class Transformer(nn.Module):
  def __init__(self, input_vocab_size, output_vocab_size, device) -> None:
    super(Transformer, self).__init__()

    self.device = device
    self.encoder = Encoder(input_vocab_size, Config.d_model)
    self.decoder = Decoder(output_vocab_size, Config.d_model)

    self.linear = nn.Linear(Config.d_model, output_vocab_size)

  def forward(self, x, y):
    enc_padding_mask, dec_padding_mask, lookahead_mask = self._create_masks(x, y)
    enc_out = self.encoder(x, enc_padding_mask)
    dec_out = self.decoder(y, enc_out, dec_padding_mask, lookahead_mask)
...
class Encoder(nn.Module):
  def __init__(self, vocab_size, device) -> None:
    ...
    self.layers = nn.Sequential(*[EncoderLayer(device) for _ in range(Config.num_layer)])
...
class EncoderLayer(nn.Module):
  def __init__(self, device) -> None:
    ...
    self.mha_block = SubLayerBlock(SelfAttention(device), device)
    self.ffn_block = SubLayerBlock(FeedForwardNetworks(device), device)

  def forward(self, input):
    x, mask = input
    out1 = self.mha_block(x, x, mask)
...
class Decoder(nn.Module):
  ...
  def forward(self, x, enc_out, pad_mask, lookahead_mask):
    embedd = self.embedding(x)
    out = self.layers((embedd, enc_out, pad_mask, lookahead_mask))
    return out[0]
...
class DecoderLayer(nn.Module):
  def __init__(self, device) -> None:
    ...
    self.enc_dec_block = SubLayerBlock(SelfAttention(device), device)
    self.ffn_block = SubLayerBlock(FeedForwardNetworks(device), device)

  def forward(self, input):
    x, enc_out, pad_mask, lookahead_mask = input
    out1 = self.masked_block(x, x, lookahead_mask)
    out2 = self.enc_dec_block(out1, enc_out, pad_mask)
    out3 = self.ffn_block(out2, None, None)
    return (out3, enc_out, pad_mask, lookahead_mask)
